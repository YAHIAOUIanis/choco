...
public class SudokuCenterDotCh {
	...
	public static void main(String[] args) {
		...
		if (n % 2 == 0) {
			System.err.println("N doit être impair");
			return;
		}
		...
		// *** SET CONSTRAINTS ON CENTER DOT ***
		IntVar[] centers = new IntVar[n2];
		int m = 0;
		for (int j = 0; j < n2; j++) {
			for (int i = 0; i < n2; i++) {
				if (i % n == 0 && j % n == 0) {
					int xMiddleZone = j + n / 2;
					int yMiddleZone = i + n / 2;
					centers[m++] = v[xMiddleZone][yMiddleZone];
				}
			}
		}
		model.allDifferent(centers).post();
